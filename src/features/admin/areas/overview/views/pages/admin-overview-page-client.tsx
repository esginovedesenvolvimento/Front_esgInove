"use client";

import { useEffect, useState } from "react";
import { AdminBoardLoading } from "@/features/admin/shared/components/admin-board-loading";
import type { AdminOverviewModel } from "@/features/admin/shared/types";
import { getAdminOverviewModel } from "../../services/overview.service";
import { AdminOverviewView } from "./admin-overview-view";

export function AdminOverviewPageClient() {
  const [model, setModel] = useState<AdminOverviewModel | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let active = true;

    getAdminOverviewModel()
      .then((data) => {
        if (active) setModel(data);
      })
      .catch((err) => {
        if (active) setError(err instanceof Error ? err.message : "Erro ao carregar visão geral");
      });

    return () => {
      active = false;
    };
  }, []);

  if (!model && !error) {
    return (
      <AdminBoardLoading
        eyebrow="Visão geral"
        title="Operação ESG em tempo real"
        description="Carregando leitura rápida da operação."
      />
    );
  }

  if (error && !model) {
    return (
      <div className="rounded-3xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700">
        {error}
      </div>
    );
  }

  return model ? <AdminOverviewView model={model} /> : null;
}
