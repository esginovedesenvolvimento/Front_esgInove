  ...
  return (
    <div className="space-y-6 pb-6 animate-in fade-in duration-700 max-h-[calc(100vh-100px)] overflow-hidden">
      ...
      <div className="grid gap-6 lg:grid-cols-12 items-start h-[calc(100vh-180px)]">
        ...
        {/* Lado Direito: Serviços Sob Demanda - COM SCROLL INFINITO */}
        <div className="lg:col-span-4 bg-slate-50/50 p-5 rounded-2xl border border-slate-200 h-full flex flex-col justify-between overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden relative pause-hover mb-4">
            <div className="animate-vertical-scroll flex flex-col gap-4 py-2">
              {/* Original items */}
              {demandServices.map((service) => {
                const Icon = service.icon;
                const inCart = isInCart(service.id);
                return (
                  <div key={service.id} className="rounded-xl border border-slate-200 bg-white p-3 flex flex-col justify-between shadow-sm hover:border-emerald-300 hover:shadow-md transition-all shrink-0">
                    <div>
              ...
              {/* Duplicated items for seamless infinite scroll loop */}
              {demandServices.map((service) => {
                const Icon = service.icon;
                const inCart = isInCart(service.id);
                return (
                  <div key={`${service.id}-dup`} className="rounded-xl border border-slate-200 bg-white p-3 flex flex-col justify-between shadow-sm hover:border-emerald-300 hover:shadow-md transition-all shrink-0">
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        <div className="p-1 rounded-lg bg-slate-100 text-slate-600">
                          <Icon className="h-3.5 w-3.5" />
            ...
            {/* Fade overlays for glassmorphic vertical visual excellence */}
            <div className="absolute top-0 left-0 right-0 h-8 bg-gradient-to-b from-slate-50/90 to-transparent pointer-events-none z-10" />
            <div className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-slate-50/90 to-transparent pointer-events-none z-10" />
          </div>
