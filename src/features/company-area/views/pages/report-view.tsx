...
export function ReportView({ model, reportType, dbDiagnostic }: ReportViewProps) {
  ...
  const socScore = model.axisScores?.[1]?.score || 64;
  const socProven = isPre ? 0 : model.axisScores?.[1]?.provenScore || 60;
  const govScore = model.axisScores?.[2]?.score || 71;
  const govProven = isPre ? 0 : model.axisScores?.[2]?.provenScore || 67;
